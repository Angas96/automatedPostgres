...
docker run -d -p $POSTGRES_PORT:5432 -e POSTGRES_USER=$POSTGRES_USER -e POSTGRES_PASSWORD=$POSTGRES_PASSWORD -e POSTGRES_DB=$POSTGRES_DB -e POSTGRES_OPENWEATHERLINK=$POSTGRES_OPENWEATHERLINK -e POSTGRES_OPENWEATHERLINK_FORECAST=$POSTGRES_OPENWEATHERLINK_FORECAST -e POSTGRES_METGISLINK_CURRENT=$POSTGRES_METGISLINK_CURRENT -e POSTGRES_METGISLINK_FORECAST=$POSTGRES_METGISLINK_FORECAST --name automatedPostgres automatedpostgres
